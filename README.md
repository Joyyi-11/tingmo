...
面向小宇宙播客的结构化 Markdown 文稿生成工具。输入小宇宙单集链接，自动抓取节目元信息、下载音频、本地转写、说话人识别、LLM 忠实校订，并输出适合阅读和编辑的 Markdown。
...
小宇宙播客成稿的难点不只是把声音变成文字，还包括长文本不中断、专名不漂移、说话人结构可读、失败后能续跑，以及保留原始证据方便回查。

听墨的处理链路是：

```text
episode URL
  -> scrape Xiaoyuzhou metadata and audio
  -> download and convert audio
  -> faster-whisper local transcript
  -> optional speaker diarization
  -> chunked LLM cleaning with JSON validation
  -> summary, quotes and speaker intro
  -> structured Markdown
```
